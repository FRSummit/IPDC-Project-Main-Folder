    ...
    <div class="content-div">
      <form class="form-cls" @submit.prevent="onSubmit">
        ...
        <v-row class="form-row">
          ...
          <router-link to='/planandreports/report_landing_page/dawah-material-distribution' style="width: 100%;">
            <v-col cols="12" class="col-field" style="margin: 20px auto; text-align: center;">
              <v-text name="comment" v-model="comment" label="Comment" placeholder="Comment" single-line 
                ...
                Go to >> Dawah Material Distribution
              </v-text>
            </v-col>
          </router-link>
        </v-row>
...
export default {
  ...
  methods: {
    onSubmit() {
      if(this.planOrReportTab === 'PLAN') {
        ...
        console.log(post)
        localStorage.setItem('teachingAndLearningProgram_otherTeachingLearningProgramPlanData', JSON.stringify(post))
      } 
      if(this.planOrReportTab === 'REPORT') {
        ...
        console.log(post)
        localStorage.setItem('teachingAndLearningProgram_otherTeachingLearningProgramData', JSON.stringify(post))
      }
    },
    backButton() {
      console.log('working')
  ...
  created() {
    ...
    if(tabActivationForPlanOrReport === 'PLAN') {
      ...
      this.target = planData.otherTeachingLearningProgramPlanData.target
      this.dateAndAction = planData.otherTeachingLearningProgramPlanData.dateAndAction
    }
    if(tabActivationForPlanOrReport === 'REPORT') {
      ...
      this.target = reportData.otherTeachingLearningProgramData.target
      this.dateAndAction = reportData.otherTeachingLearningProgramData.dateAndAction
      this.actual = reportData.otherTeachingLearningProgramData.actual
      this.averageAttendance = reportData.otherTeachingLearningProgramData.averageAttendance
      this.comment = reportData.otherTeachingLearningProgramData.comment
    }
  }
}
</script>
