...
export default {
  ...
  methods: {
    onSubmit() {
      if(this.planOrReportTab === 'PLAN') {
        ...
        console.log(post)
        localStorage.setItem('manpowerAndPersonalContacts_preliminaryMemberPlanData', JSON.stringify(post))
        ...
        console.log(post)
        localStorage.setItem('manpowerAndPersonalContacts_preliminaryMemberData', JSON.stringify(post))
      }
    },
    backButton() {
      console.log('working')
  ...
  created() {
    ...
    if(tabActivationForPlanOrReport === 'PLAN') {
      ...
      this.nameAndContactNumber = planData.preliminaryMemberPlanData.nameAndContactNumber
      this.action = planData.preliminaryMemberPlanData.action
      this.target = planData.preliminaryMemberPlanData.upgradeTarget
      ...
      this.nameAndContactNumber = reportData.preliminaryMemberData.nameAndContactNumber
      this.action = reportData.preliminaryMemberData.action
      this.lastPeriod = reportData.preliminaryMemberData.lastPeriod
      this.upgradeTarget = reportData.preliminaryMemberData.upgradeTarget
      this.thisPeriod = reportData.preliminaryMemberData.thisPeriod
      this.increased = reportData.preliminaryMemberData.increased
      this.decreased = reportData.preliminaryMemberData.decreased
      this.comment = reportData.preliminaryMemberData.comment
      this.memberContact = reportData.preliminaryMemberData.personalContact
    }
  }
}
</script>
