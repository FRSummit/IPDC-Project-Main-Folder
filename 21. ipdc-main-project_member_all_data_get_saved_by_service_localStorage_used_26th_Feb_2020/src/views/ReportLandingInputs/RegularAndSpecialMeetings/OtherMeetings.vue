    ...
    <div class="content-div">
      <form class="form-cls" @submit.prevent="onSubmit">
        ...
        <v-row class="form-row">
          ...
          <router-link to='/planandreports/report_landing_page/teaching-and-learning-program' style="width: 100%;">
            <v-col cols="12" class="col-field" style="margin: 20px auto; text-align: center;">
              <v-text name="comment" v-model="comment" label="Comment" placeholder="Comment" single-line 
                ...
                Go to >> Teaching &amp; Learning Program
              </v-text>
            </v-col>
          </router-link>
        </v-row>
...
export default {
  ...
  methods: {
    onSubmit() {
      if(this.planOrReportTab === 'PLAN') {
        ...
        console.log(post)
        localStorage.setItem('regularAndSpecialMeetings_otherMeetingProgramPlanData', JSON.stringify(post))
        ...
        console.log(post)
        localStorage.setItem('regularAndSpecialMeetings_otherMeetingProgramData', JSON.stringify(post))
      }
    },
  ...
  created() {
    ...
    if(tabActivationForPlanOrReport === 'PLAN') {
      ...
      this.target = planData.otherMeetingProgramPlanData.target
      this.dateAndAction = planData.otherMeetingProgramPlanData.dateAndAction
      ...
      this.target = reportData.otherMeetingProgramData.target
      this.dateAndAction = reportData.otherMeetingProgramData.dateAndAction
      this.actual = reportData.otherMeetingProgramData.actual
      this.averageAttendance = reportData.otherMeetingProgramData.averageAttendance
      this.comment = reportData.otherMeetingProgramData.comment
    }
  }
}
</script>
