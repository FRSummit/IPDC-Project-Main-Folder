...
export default {
  ...
  methods: {
    onSubmit() {
      if(this.planOrReportTab === 'PLAN') {
        ...
        console.log(post)
        localStorage.setItem('teachingAndLearningProgram_memorizingDoaTeachingLearningProgramPlanData', JSON.stringify(post))
      } 
      if(this.planOrReportTab === 'REPORT') {
        ...
        console.log(post)
        localStorage.setItem('teachingAndLearningProgram_memorizingDoaTeachingLearningProgramData', JSON.stringify(post))
      }
    },
    backButton() {
      console.log('working')
  ...
  created() {
    ...
    if(tabActivationForPlanOrReport === 'PLAN') {
      ...
      this.target = planData.memorizingDoaTeachingLearningProgramPlanData.target
      this.dateAndAction = planData.memorizingDoaTeachingLearningProgramPlanData.dateAndAction
    }
    if(tabActivationForPlanOrReport === 'REPORT') {
      ...
      this.target = reportData.memorizingDoaTeachingLearningProgramData.target
      this.dateAndAction = reportData.memorizingDoaTeachingLearningProgramData.dateAndAction
      this.actual = reportData.memorizingDoaTeachingLearningProgramData.actual
      this.averageAttendance = reportData.memorizingDoaTeachingLearningProgramData.averageAttendance
      this.comment = reportData.memorizingDoaTeachingLearningProgramData.comment
    }
  }
}
</script>
